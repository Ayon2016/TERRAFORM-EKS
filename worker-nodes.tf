...
resource "aws_eks_node_group" "mynodeGroup" {
  ...
  lifecycle {
    # Tests that tag is not null
    # Tests that the AMI type is ON_DEMAND

    postcondition {
      condition     = self.tags != null
      error_message = "Tags should not be null"
    }
    postcondition {
...
locals {
  default_config = yamldecode(file("values.yaml"))
}

resource "aws_iam_role_policy_attachment" "node_AmazonEKSWorkerNodePolicy" {
  policy_arn = local.default_config["policy_arn"]
  role       = aws_iam_role.node.name
}
